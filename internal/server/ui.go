...
async function load(){var r=await fetch(A+'/audit_entries').then(function(r){return r.json()});items=r.audit_entries||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.actor||'').toLowerCase().includes(q)||(i.resource||'').toLowerCase().includes(q)||(i.details||'').toLowerCase().includes(q)});
...
if(i.actor)h+='<div class="item-sub">'+esc(i.actor)+'</div>';
...
if(i.resource)h+='<span>'+esc(i.resource)+'</span>';
if(i.details)h+='<span>'+esc(i.details)+'</span>';
if(i.ip_address)h+='<span>'+esc(i.ip_address)+'</span>';
if(i.source)h+='<span>'+esc(i.source)+'</span>';
if(i.severity)h+='<span>'+esc(i.severity)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/audit_entries/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",actor:"",resource:"",details:"",ip_address:"",source:"",severity:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+'</h2>';
h+='<div class="fr"><label>Action *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Actor</label><input id="f-actor" value="'+esc(i.actor)+'"></div>';
h+='<div class="fr"><label>Resource</label><input id="f-resource" value="'+esc(i.resource)+'"></div>';
h+='<div class="fr"><label>Details</label><input id="f-details" value="'+esc(i.details)+'"></div>';
h+='<div class="fr"><label>I P Address</label><input id="f-ip-address" value="'+esc(i.ip_address)+'"></div>';
h+='<div class="fr"><label>Source</label><input id="f-source" value="'+esc(i.source)+'"></div>';
h+='<div class="fr"><label>Severity</label><input id="f-severity" value="'+esc(i.severity)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),actor:document.getElementById('f-actor').value.trim(),resource:document.getElementById('f-resource').value.trim(),details:document.getElementById('f-details').value.trim(),ip_address:document.getElementById('f-ip-address').value.trim(),source:document.getElementById('f-source').value.trim(),severity:document.getElementById('f-severity').value.trim()};
if(editId){await fetch(A+'/audit_entries/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/audit_entries',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
