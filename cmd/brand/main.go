...
func main(){port:=os.Getenv("PORT");if port==""{port="9800"};dataDir:=os.Getenv("DATA_DIR");if dataDir==""{dataDir="./brand-data"}
db,err:=store.Open(dataDir);if err!=nil{log.Fatalf("brand: %v",err)};defer db.Close();srv:=server.New(db)
